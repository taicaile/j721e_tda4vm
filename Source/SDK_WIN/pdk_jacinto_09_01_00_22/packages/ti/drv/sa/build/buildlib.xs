 ...
 *
 * DESCRIPTION: 
 *  This file contains common routines that are used by the various SA 
 *  components.
 *
...
function createMake(makefile)
{
    ...
    if(makefile==undefined)
    {
      ...
      for (var i = 0; i < keys.length; i++) {
           key = String(keys[i]);
           if((key.match("INSTALL_PATH")) || (key.match("INSTALLDIR")) || (key.match("INSTALL_DIR")))
           {
      ...
      makefile.writeLine("\t$(RMDIR) $(LIBDIR)/*");  
...
function createLibMake(makelibname,targetname, objectPath, objloc)
{
   var tooldir;
   var cmdprefix;
   var targetDir;
   ...
   switch(stringname)
   {
    ...
    if ((targetname.name == "A15F") || (targetname.name == "A9F") || (targetname.name == "A8F"))
    {
        ...
        libmakefile.writeLine("OBJDIR =  $(LIBDIR)/" + targetDir +"/obj"+objloc+targetname.suffix); 
 ...
 **************************************************************************/
var makefilelocal;
function buildLibrary (libOptions, libName, target, libFiles, objloc) 
{
    var targetDir;
    ...
    var makefilelib= createLibMake(lib+".mk",target,objectPath, objloc);  
